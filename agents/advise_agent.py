...
SYSTEM_PROMPT = (
    ...
    "YOU REWRITE DRIVER FEEDBACK MESSAGES."
    "Be concise. No extra disclaimers. One short tip about the driver."
    "Example of answer: You are cautious in a risk zone."
)
...
def _rule_draft(policy: PolicyState, alerts: List[Alert]) -> str:
    ...
    if beh.lower()=="cautious" and risk!="none":
        return "Cautious driving in a risk zone. Keep attention."
...
async def advise_agent(
    ...
    user = (
        f"{draft}\n Behavior: {behavior}. PRF Zone: {risk}."
    ...
    try:
        ...
        # -----------------------------------------------
        # Detecta se o LLM incluiu Behavior/PRF zone
        # -----------------------------------------------
        low = text.lower()
        has_behavior = behavior.lower() in low
        
        if risk is not None:
            has_prf = ("prf zone" in low) or ("accident" in low) or ("fine" in low)
