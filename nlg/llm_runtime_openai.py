...
class LLMRuntimeOpenAI:
    def __init__(self, base_url: str, model: str, max_tokens: int = 64, temperature: float = 0.1, top_p: float = 0.9, timeout_s: float = 6.0,
                monitor_pid: Optional[int] = None, sample_interval_s: float = 0.1, repeat_penalty: float=1.1, track_emissions: bool = False):
    ...
    async def chat(self, system: str, user: str) -> Dict[str, Any]:
        ...
        if self.track_emissions:
            try:
                ...
                # não deixa isso quebrar a inferência
                print(f"[WARN] CodeCarbon start failed: {e}")
                tracker = None
