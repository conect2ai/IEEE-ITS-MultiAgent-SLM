...
REPLAY_CSV="./replays/trip_log_etios_0.csv"
...
def compute_features_and_predictions(raw, rec: RowMetrics | None = None):
    ...
    raw['accel_magnitude'] = np.sqrt(raw["accel_x"]**2 + raw["accel_y"]**2 + raw["accel_z"]**2)
...
async def _startup():
    ...
    TRIP_LOG_FILE = init_trip_log(base_dir=os.getenv("TRIP_LOG_DIR", "./data/trips"))
    ...
    while True:
        try:
            ...
            if ORCH is not None:
                ...
                orch_run_ms = (t_run_end - t_run_start) * 1000.0
            ...
            orch_llm_enqueue_ms = 0.0
            if enqueue_policy is not None:
                t_enq_start = time.monotonic()
                await ORCH.enqueue_llm_job(
                    rid,
                    enqueue_policy,
                    enqueue_alerts,
                ...
                t_enq_end = time.monotonic()
                orch_llm_enqueue_ms = (t_enq_end - t_enq_start) * 1000.0

            processed["orch_run_ms"] = orch_run_ms
            processed["orch_llm_enqueue_ms"] = orch_llm_enqueue_ms
